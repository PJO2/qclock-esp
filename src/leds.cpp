...
// leds.cpp
// 
// display a "word" (ie a position and a length) on the led panel
// -------------------------------------------------------------


#include <Adafruit_NeoPixel.h>

#include "time2words.h"   // retrieve S_Word structure
#include "leds.h"
...
// Some precalculated colors for each position
struct S_Degrade
{
   byte r;
   byte g;
...
const struct S_Degrade tColors[] =
{   
   ...
   { 138, 246, 118,  0 },     { 136, 248, 120,  0 },     { 134, 250, 122,  0 },     { 132, 252, 124,  0 },     { 130, 254, 126,  0 },     { 128, 255, 128,  0 },     { 126, 254, 130,  0 },  
...
// initialization 
// 
void initLeds (int strip_pin)
{
   strip = new Adafruit_NeoPixel (NUMPIXELS, strip_pin, NEO_GRB + NEO_KHZ800);                 
...
static void display_word (Adafruit_NeoPixel *strip, int pos, int len)
{
   for (int i=pos ; i<pos + len ; i++)
         strip->setPixelColor ( i ,  pgm_read_dword (tColors + i) );
} // display_word



// display multiples words 
void display_multiple_words (const struct s_Word *tword, int len)
{
  strip->clear();
  for (int ark=0 ;  ark<len ; ark++)
      display_word (strip, tword[ark].pos, tword[ark].len);
  strip->show();
} // display_multiple_words 
